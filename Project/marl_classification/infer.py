...
from .utils import get_visualize_attention, show_cam_on_image
...
def visualize_steps(
    ...
    img_ori = denomalizer(img_ori)
    img_ori, attn = get_visualize_attention(vit, img_ori, patch_size=8,device="cuda")
    vis = show_cam_on_image(img_ori.cpu().numpy().transpose(1,2,0), attn.cpu().numpy().transpose(1,2,0))
