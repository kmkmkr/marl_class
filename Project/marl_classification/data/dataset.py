...
class MNISTDataset(ImageFolder):
    def __init__(
        self, res_path: str, img_transform: Callable[[Any], th.Tensor], use_attn_reward: bool, attn_reward_path: str
    ) -> None:
        ...
        if use_attn_reward:
            with open(attn_reward_path, 'rb') as f:
                self.attn = pkl.load(f)["attn"]
        self.use_attn_reward = use_attn_reward
    ...
    def __getitem__(self, index: int) -> Tuple[th.Tensor, th.Tensor]:
        ...
        if self.use_attn_reward:
            return img, th.tensor(label), self.attn[index]
